...
namespace Esoft_Project
{
    public partial class FormDeal : Form
    {
        ...
        void Deductions()
        {
            if (comboBoxSupply.SelectedIndex != null && comboBoxDemand.SelectedIndex != null)
            {
                ...
                double customerCompanyDeductions = supplySet.Pirce * 0.03;
            ...
            if (comboBoxSupply.SelectedItem != null)
            {
                ...
                {
                    sellerCompanyDeductions = 36000 + supplySet.Pirce * 0.01;
                    textBoxSellerCompanyDeductions.Text = sellerCompanyDeductions.ToString("0.00");
                }
                else if (supplySet.RealEstateSet.Type == 1)
                {
                    sellerCompanyDeductions = 30000 + supplySet.Pirce * 0.01;
                    textBoxSellerCompanyDeductions.Text = sellerCompanyDeductions.ToString("0.00");
                }
                else
                {
                    sellerCompanyDeductions = 30000 + supplySet.Pirce * 0.02;
                    textBoxSellerCompanyDeductions.Text = sellerCompanyDeductions.ToString("0.00");
                }
                if (supplySet.AgentsSet.DealShare != null)
                {
                    ...
                    textBoxAgentSellerDedictions.Text = agentSellerDeductions.ToString("0.00");
                }
                else
                {
            ...
            else
            {
                ...
                textBoxAgentSellerDedictions.Text = "";
                textBoxCustomerCompanyDeductions.Text = "";
                textBoxAgentSellerDedictions.Text = "";
            }            
        }
        void ShowDealSet()
        {
            ...
            foreach (DealSet deal in Program.wftDb.DealSet)
            {
                ListViewItem item = new ListViewItem(new string[]
                {
                        ...
                        "г. "+deal.SupplySet.RealEstateSet.Address_City+", ул. "+deal.SupplySet.RealEstateSet.Address_Street+", д. "+deal.SupplySet.RealEstateSet.Address_House+", кв. "+deal.SupplySet.RealEstateSet.Address_Number, deal.SupplySet.Pirce.ToString()
                });
                item.Tag = deal;
